...
起点是一份开源的 ScreenAgent 训练配方（模型选型、数据集、超参数），但把它跑起来只是第一步——评估分数长期卡在接近 0 且完全不报错，从这里开始的排查、诊断实验设计、根因定位、架构修复、训练/停止策略，都是独立完成的实验工作，不在原配方覆盖范围内。完整过程、每一步的证据和最终技术报告见 **[docs/REPORT.md](docs/REPORT.md)**。
...
这不是"下载数据、跑脚本、等结果"式的复现，下面这几件事是本项目独立设计和实现的，完整推导过程和量化证据见 [docs/REPORT.md](docs/REPORT.md)（开头的"核心创新点"表格可快速浏览）：
